...
class MMDoubleStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        mlp_hidden_dim = int(hidden_size * mlp_width_ratio) # 12288
...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        for i_block_here, block in enumerate(self.double_blocks):
            double_block_args = [
                img,
                txt,
                vec,
        ...
        if len(self.single_blocks) > 0:
            for i_block_here, block in enumerate(self.single_blocks):
                single_block_args = [
                    x,
                    vec,
                    txt_seq_len,
