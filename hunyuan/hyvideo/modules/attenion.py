...
def generate_restore_indices(
    ...
    """
    Build the inverse of the reorder index list, so that
      restored = reorganized[:, restore_idx, ...]
    yields the original ordering.
    """
    reorg_idx = generate_reorg_indices(
        total_len, part_size, block_size, sub_block_size
    )
    # invert the mapping
    restore_idx = [0] * total_len
...
length_here = 126_720
reorg_idx = generate_reorg_indices(length_here)
restore_idx = generate_restore_indices(length_here)
text_len = 256
reorg_idx += [i for i in range(length_here, text_len + length_here)]
restore_idx += [i for i in range(length_here, text_len + length_here)]
...
def attention(
    ...
    # Xuan: implement the block sparse attention here
        ...
        # ===============================================================================
        streaming_info = None
        k_len, num_heads, head_dim = k.shape
        q_len, _, _ = q.shape
        # Xuan: in 'head_mask_type': torch.tensor([x] * 24, device=q.device, dtype=torch.int32) (PS: in hunyuan, there are 24 heads)
        # mask_type = 0 denotes the dense attention
        # mask_type = -1 denotes the streaming attention
        # mask_type = 1 denotes the blocksparse attention, and the head will use basemask[mask_type - 1] as its attention mask
        head_mask_type = torch.tensor([1] * num_heads, device=q.device, dtype=torch.int32)

        # sparse blockmask
        # ===============================================================================
        sparsity_ratio = 0.8 # todo
        attn = sample_qk_attention_2d(
            q[:126720], k[:126720],
            frame_h=48,
            frame_w=80,
            pool_h=8,
        ...
        m_block_dim = (126720 + attn.shape[1] - 1) // attn.shape[1] # 128
        n_block_dim = (126720 + attn.shape[2] - 1) // attn.shape[2] # 128
        q_block_num = (max_seqlen_q + m_block_dim - 1) // m_block_dim
        k_block_num = (max_seqlen_kv + n_block_dim - 1) // n_block_dim
        base_blockmask_visual = attention_percentile_mask_headwise(attn, 1-sparsity_ratio)
        ...
        # k_block_num = (max_seqlen_kv + n_block_dim - 1) // n_block_dim
        # base_blockmask = torch.ones(
        #     (cu_seqlens_q.numel() - 1, num_heads, q_block_num, k_block_num),
        #     device=q.device,
        #     dtype=q.dtype,
        # ).bool()
        # ===============================================================================
    ...
    # Xuan: implement the block sparse attention here
    elif mode == "block_sparse_attention_dense":
        # ===============================================================================
        # ===============================================================================
        streaming_info = None
        k_len, num_heads, head_dim = k.shape
