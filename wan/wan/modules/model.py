...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, idx_block=None, current_timestep=None):
        r"""
        Args:
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # self-attention
        y = self.self_attn(     # Xuan: self-attention
            self.norm1(x).float() * (1 + e[1]) + e[0], seq_lens, grid_sizes,
            freqs,
            idx_block, current_timestep)
        with amp.autocast(dtype=torch.float32):
            x = x + y * e[2]
