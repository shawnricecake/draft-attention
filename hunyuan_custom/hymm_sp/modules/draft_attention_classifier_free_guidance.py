...
class Draft_Attention(nn.Module):
    def __init__(
        ...
        assert batch_size == 2, "only support batch size 2 for now, i.e., classifier-free guideance"
    ...
    def generate_reorg_restore_indices(
        ...
        # add the batch size to the reorg and restore indices
        if batch_size > 1:
            reorg_idx += [visual_len + text_len + reorg_idx[i] for i in range(visual_len + text_len)]
            restore_idx += [visual_len + text_len + restore_idx[i] for i in range(visual_len + text_len)]
    ...
    def sample_qk_attention_2d(
        ...
        assert L % frame_tokens == 0, "L must be multiple of frame_h*frame_w, L={} frame_h={} frame_w={}".format(L, frame_h, frame_w)
    ...
    def forward(
        ...
        attn_1 = self.sample_qk_attention_2d(
            q[:self.visual_len], 
            k[:self.visual_len],
        ...
        attn_2 = self.sample_qk_attention_2d(
            q[int(self.visual_len + self.text_len):int(self.visual_len * batch_size + self.text_len)], 
            k[int(self.visual_len + self.text_len):int(self.visual_len * batch_size + self.text_len)],
            frame_h=self.latent_h,
            frame_w=self.latent_w,
            pool_h=self.pool_h,
            pool_w=self.pool_w,
        ...
        m_block_dim = (self.visual_len + attn_1.shape[1] - 1) // attn_1.shape[1] # 128
        n_block_dim = (self.visual_len + attn_1.shape[2] - 1) // attn_1.shape[2] # 128
        q_block_num = (max_seqlen_q + m_block_dim - 1) // m_block_dim
        k_block_num = (max_seqlen_kv + n_block_dim - 1) // n_block_dim
        base_blockmask_visual_1 = self.attention_percentile_mask_headwise(attn_1, 1-sparsity_ratio)
        base_blockmask_visual_2 = self.attention_percentile_mask_headwise(attn_2, 1-sparsity_ratio)
        base_blockmask_visual_1 = base_blockmask_visual_1.unsqueeze(0).repeat(
            int((cu_seqlens_q.numel() - 1) / 2), 1, 1, 1
        ).bool()
        base_blockmask_visual_2 = base_blockmask_visual_2.unsqueeze(0).repeat(
            int((cu_seqlens_q.numel() - 1) / 2), 1, 1, 1
        ).bool()
        ...
        base_blockmask[:int((cu_seqlens_q.numel() - 1) / 2), :, :base_blockmask_visual_1.shape[2], :base_blockmask_visual_1.shape[3]] = base_blockmask_visual_1
        base_blockmask[int((cu_seqlens_q.numel() - 1) / 2):, :, :base_blockmask_visual_2.shape[2], :base_blockmask_visual_2.shape[3]] = base_blockmask_visual_2
